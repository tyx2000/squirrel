// Purpose: Coordinates window screen recording, recording options, and MP4 file output.
...
final class ScreenRecordingService: ObservableObject {
    ...
    func toggleWindowRecording(onFailure: @escaping (String) -> Void) {
        ...
        guard !isPreparingRecording else {
            fail("Record Window is still preparing.", onFailure: onFailure)
            return
        }

        startWindowRecording(onFailure: onFailure)
    }

    private func startWindowRecording(onFailure: @escaping (String) -> Void) {
        ...
        Task { @MainActor in
            ...
            do {
                ...
                guard !content.displays.isEmpty || !content.windows.isEmpty else {
                    fail("Record Window found no screens or windows to record.", onFailure: onFailure)
                    return
                }
                ...
                fail("Record Window failed: \(error.localizedDescription)", onFailure: onFailure)
            }
        }
    }
    ...
    private func stopRecording(onFailure: @escaping (String) -> Void) {
        ...
        Task { @MainActor in
            do {
                ...
                fail("Record Window could not stop recording: \(error.localizedDescription)", onFailure: onFailure)
            }
        }
    }
    ...
    private func beginRecording(_ request: RecordingRequest) async throws {
        ...
        delegate.onFailure = { [weak self, weak recording] error in
            Task { @MainActor in
                ...
                self.isRecording = false
                self.outputURL = nil
                self.lastMessage = "Record Window failed: \(error.localizedDescription)"
            }
        }
    ...
    private func presentRecordingRequest(for content: RecordableContent) -> RecordingRequest? {
        ...
        alert.informativeText = "Choose full screen or a window to record. The recording will be saved to Downloads."
    ...
    private func showScreenRecordingGuide(onFailure: @escaping (String) -> Void) {
        let message = "Record Window requires Screen Recording access in System Settings."
...
private enum RecordingError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .couldNotAddRecordingOutput:
            "ScreenCaptureKit did not accept the recording output."
        }
    }
}
